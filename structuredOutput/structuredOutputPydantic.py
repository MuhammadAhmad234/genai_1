...
# Annotated is used to provide a def of the schema attribute 

class Review(BaseModel):

    key_themes: list[str] = Field(description= "List all the key themes that are discussed in the review")
    pros: list[str] = Field(description= "List all the pros mentioned in the review, you can skip them if pros are not available thery are optional data")
    cons: list[str] = Field(description= "List all the cons mentioned in the review, you can skip them if cons are not available thery are optional data")
...
result = structured_model.invoke(
    ...
    However, the weight and size make it a bit uncomfortable for one-handed use. Also, Samsung's One
    UI still comes with bloatware-why do I need five different Samsung apps for things Google already
